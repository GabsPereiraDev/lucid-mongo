...
test.group('Relations | Has Many', (group) => {
  ...
  test('throw exception when createMany doesn\'t receives an array', async (assert) => {
    ...
    try {
      ...
      assert.match(message, /E_INVALID_PARAMETER: hasMany.createMany expects an array of values instead received object/)
    }
  })
  ...
  test('throw exception when saveMany doesn\'t receives an array', async (assert) => {
    ...
    try {
      ...
      assert.match(message, /E_INVALID_PARAMETER: hasMany.saveMany expects an array of related model instances instead received object/)
    }
  })
})
