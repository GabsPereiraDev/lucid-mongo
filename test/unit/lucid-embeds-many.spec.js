...
test.group('Relations | Embeds many', (group) => {
  ...
  test('find by id related instance', async (assert) => {
    ...
    await ioc.use('Database').collection('users').insert({ username: 'virk', emails: [{ _id: new ObjectId(), address: 'example@gmail.com' }, { _id: new ObjectId(), address: 'example2@gmail.com' }] })
    const user = await User.first()
    assert.instanceOf(user, User)
